...
public interface DictionaryBucketPaymentRepository extends JpaRepository<DictionaryBucketPayment, String>, JpaSpecificationExecutor<DictionaryBucketPayment> {
    @Query(
    value = "SELECT er.id, er.expense_date AS date, amount,de.icon_url, de.name AS category_name, 'expense' AS TYPE, interpretation FROM expense_regular er\n" +
            ...
            "UNION\n" +
            "SELECT rr.id, rr.revenue_date AS date,amount, dr.icon_url, dr.name AS category_name, 'revenue' AS TYPE, interpretation FROM revenue_regular rr\n" +
