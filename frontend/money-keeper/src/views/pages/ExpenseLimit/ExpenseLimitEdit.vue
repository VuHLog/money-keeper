...
async function updateExpenseLimit() {
    if (!isValid()) {
        return;
    }
...
async function deleteExpenseLimit() {
    const result = await swal.fire({
        title: "Xác nhận xóa",
        text: "Bạn có chắc chắn muốn xóa hạn mức chi này?",
        icon: "warning",
    ...
    if (result.isConfirmed) {
        try {
            ...
            swal.fire({
                title: "Lỗi",
                text: "Không thể xóa hạn mức chi",
                icon: "error",
            });
        }
    }
    ...
    <div class="text-grey-color">
        <v-container fluid class="pa-0">
            <v-row align="center" class="ma-0">
                ...
                <v-col>
                    <h1 class="text-h4 text-center text-primary mb-0">Chỉnh sửa hạn mức chi</h1>
                </v-col>
            </v-row>
        </v-container>
        <div class="my-10">
                ...
                <v-col cols="3">
                    <v-btn class="cursor-pointer" elevation="4" rounded="xl" size="x-large" @click="showAccountModal = true">
                        <template v-if="account.length > 0">
                            ...
                            <template v-for="(item,index) in account" :key="item">
                                <span v-if="index === 0" class="text-14" style="text-transform: none">{{ item.accountName }}</span>
                                <span v-if="index === 1" class="text-14" style="text-transform: none">{{ ", " + item.accountName }}</span>
                            </template>
                            <span v-if="account.length > 2" class="text-14" style="text-transform: none">{{ " + " + (account.length - 2) + " tài khoản khác" }}</span>
        ...
        <div class="text-center">
            <button class="bg-primary-color text-white py-2 px-10 rounded d-inline-flex justify-center mr-4"
                @click.stop="updateExpenseLimit">
                <div class="mr-2">
                    <font-awesome-icon :icon="['fas', 'floppy-disk']" />
                </div>
                Lưu
            </button>
            <button class="bg-error text-white py-2 px-10 rounded d-inline-flex justify-center"
                @click.stop="deleteExpenseLimit">
                <div class="mr-2">
